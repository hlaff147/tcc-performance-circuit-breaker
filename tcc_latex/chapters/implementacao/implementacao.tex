...
\begin{lstlisting}[language=bash, caption={Exemplo de chamada cURL ao endpoint de pagamento}, label={lst:curl-pagar}]
...
\begin{itemize}
    ...
    \item \textbf{v2 (Circuit Breaker):} Retorna \texttt{200 OK} em sucesso. Quando o circuito está \textbf{OPEN}, retorna \texttt{202 Accepted} via fallback (degradação graciosa). \textbf{Importante:} antes do circuito abrir, erros são propagados como \texttt{500} para contabilização correta das falhas pelo CB.
...
\begin{lstlisting}[language=yaml, caption={Configuração do Feign -- v1}, label={lst:v1-config}]
server:
  port: 8080
...
public PaymentResponse processPayment(String modo, PaymentRequest request) {
    ResponseEntity<String> response = acquirerClient.autorizarPagamento(modo, request.toMap());
    // 503 do adquirente -> lanca excecao para contabilizar no CB
    if (response.getStatusCode() == HttpStatus.SERVICE_UNAVAILABLE) {
        failureCounter.increment();
        throw new RuntimeException("Servico adquirente indisponivel: " + response.getBody());
    }
...
O método de fallback (Código \ref{lst:v2-fallback}) trata a abertura do circuito. \textbf{Importante:} o fallback só retorna HTTP 202 (degradação graciosa) quando o circuito está OPEN. Antes de abrir, erros são propagados como 5xx para contabilização correta pelo Circuit Breaker.
...
public PaymentResponse processPaymentFallback(String modo, PaymentRequest request, Throwable t) {
    if (t instanceof CallNotPermittedException) {
        ...
        return PaymentResponse.circuitBreakerOpen(); // HTTP 202 - fallback
    }
    // Fora do cenario de circuito OPEN: propagar erro (5xx)
    // para contabilizar falhas antes da abertura do CB
    failureCounter.increment();
...
\begin{lstlisting}[language=yaml, caption={Trecho do docker-compose.yml -- mapeamento de portas}, label={lst:docker-compose}]
