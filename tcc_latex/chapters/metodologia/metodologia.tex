...
\begin{itemize}
    ...
    \item \textbf{V3: Circuit Breaker + Time Limiter:} extensão da V2 que adiciona um limite de tempo (\textit{Time Limiter}) às chamadas. Requisições que excedem o timeout configurado são automaticamente interrompidas, forçando falhas que contribuem para a abertura do circuito. Essa configuração é particularmente eficaz em cenários de latência elevada, onde a dependência responde lentamente mas não falhando explicitamente.
\end{itemize}
...
\input{chapters/metodologia/pos_processamento_paralelismo}
...
\subsection{Cenário 6 — Falhas Intermitentes com Latência (V3)}

\textbf{Contexto Real:} Este cenário é projetado especificamente para demonstrar o valor da versão V3 (com Time Limiter). Simula uma dependência que alterna entre respostas normais, respostas lentas e falhas explícitas — um padrão comum em serviços sobrecarregados.

\begin{itemize}
    \item \textbf{Descrição:} Combina falhas intermitentes com períodos de alta latência, testando a eficácia do Time Limiter em forçar a abertura do circuito quando a dependência está lenta mas não falhando explicitamente.
    ...
    \begin{itemize}
        ...
        \item \textit{V3 (CB + Time Limiter):} O Time Limiter interrompe requisições lentas, gerando falhas que acionam o CB. Demonstra proteção superior em cenários de latência.
    \end{itemize}
