...
Os testes de carga foram executados usando \texttt{k6} e \texttt{Docker Compose}. Cada versão do \texttt{servico-pagamento} (V1-Baseline, V2-CircuitBreaker, V3-Retry) foi submetida aos cinco cenários de estresse.

Para clarificar a análise dos dados, definimos as seguintes categorias de resposta:
\begin{itemize}
    \item \textbf{Sucesso Real (HTTP 200/201):} Requisição processada com sucesso pela dependência externa.
    \item \textbf{Sucesso por Degradação (HTTP 202):} Requisição interceptada pelo Circuit Breaker e tratada via fallback (apenas V2).
    \item \textbf{Disponibilidade:} Soma de Sucesso Real e Sucesso por Degradação. Representa a capacidade do sistema de responder aos usuários sem erro.
\end{itemize}

Os resultados foram avaliados contra os \texttt{thresholds} definidos.
...
Tamanho do Efeito (Cliff's Delta) & 0,38 (médio) \\
\midrule
...
O valor observado de Cliff's Delta = 0,38 corresponde a um efeito \textbf{médio}. Embora menor que o valor de Cohen's d (que indicaria efeito grande), o Cliff's Delta é a medida mais conservadora e apropriada para dados não-paramétricos com muitos outliers, confirmando que o Circuit Breaker oferece uma melhoria consistente e robusta na resiliência do sistema, independentemente da distribuição dos dados.
