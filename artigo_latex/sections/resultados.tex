...
\textbf{Load Amplification and Thundering Herd:}
A critical comparison between V2 (CB) and V3 (Retry) reveals the danger of simple retry loops. In the \textbf{Extreme Unavailability} scenario, while V2 protects the provider by failing fast, V3 maintains persistent pressure. Although our localized experiment showed a Load Amplification Factor ($L_f$) near 1.0 due to k6's request rate limiting, in real-world "thundering herd" scenarios, the cumulative effect of exponential retries across thousands of clients can prevent a recovering service from ever becoming fully operational.
