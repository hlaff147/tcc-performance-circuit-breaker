...
\begin{table}[!t]
\centering
\caption{Consolidated Perceived Availability: V1 vs V2 by Scenario}
...
\begin{table}[!t]
\centering
\caption{Perceived Availability Comparison: V1 vs V2 vs V3}
...
\vspace{1em}
	...
	extbf{Critical Observations on Retry:} Retry provides limited gains under persistent failures and can increase latency due to repeated attempts, potentially amplifying load on an already degraded dependency.
...
The Circuit Breaker achieves these results by transforming HTTP 500 errors into HTTP 202 (fallback) responses. In the Catastrophe scenario, 62.1\% of V2 responses came from fallback, effectively converting fatal errors into meaningful outcomes for end users.
